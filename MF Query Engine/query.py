...
def query():
  try:
    ...
    #====================================================================================
    mf_structure = {'prod': None, 'month': None, '1_avg_quant': None}
    mf_type = {'prod': 'str', 'month': 'int', '1_avg_quant': 'float'}
    ...
    for (key_prod, key_month) in group:
      for row in rows:
        ...
        quant = row[6]
        try:
          if group[(key_prod, key_month)]["prod"] == prod and group[(key_prod, key_month)]["month"] <= month + 3 and group[(key_prod, key_month)]["month"] >= month - 1:
    ...
    print(title_formatter.format("prod", "month", "1_avg_quant"))

    formatter = Formatter()
    for val in group.values():
      data = {"col1": val["prod"], "col2": val["month"], "col3": val["1_avg_quant"]}
